...
# We always publish `dharithri-codec` and `dharithri-codec-derive` together.
# `framework/*` depend on both `dharithri-codec` and `dharithri-chain-scenario-format`,
...
# `/home/andreim/dharithri/rs/mx-sdk-rs/framework/meta/src/sc_upgrade/upgrade_versions.rs`
# 
...
# 
# 8. Search for `dharithri` on `crates.io` and check that the new versions appear for all crates.
...
#
# 10. Go to https://github.com/dharithri/mx-sdk-rs/tags
