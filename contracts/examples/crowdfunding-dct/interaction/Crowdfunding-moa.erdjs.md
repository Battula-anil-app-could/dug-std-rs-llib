...
let { erdSys, Moa, wallets: { alice, bob, carol }} = await erdjs.setupInteractive("local-testnet");
...
erdjs.print(Moa.raw(await crowdfunding.query.get_target()));
...
let aliceBalanceBefore = await erdSys.getBalance(alice, Moa);
...
let aliceBalanceAfter = await erdSys.getBalance(alice, Moa);
