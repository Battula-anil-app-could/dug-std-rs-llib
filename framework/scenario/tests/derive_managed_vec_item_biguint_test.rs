...
pub struct ManagedStructWithBigUint<M: ManagedTypeApi> {
    pub big_uint: dharithri_sc::types::BigUint<M>,
    pub num: u32,
}
...
fn struct_with_numbers_static() {
    assert_eq!(
        <ManagedStructWithBigUint<StaticApi> as dharithri_sc::types::ManagedVecItem>::PAYLOAD_SIZE,
        8
    );
    assert!(
        !<ManagedStructWithBigUint<StaticApi> as dharithri_sc::types::ManagedVecItem>::SKIPS_RESERIALIZATION
    );
}
...
fn managed_struct_to_bytes_writer() {
    ...
    <ManagedStructWithBigUint<StaticApi> as dharithri_sc::types::ManagedVecItem>::to_byte_writer(
        &s,
        |bytes| {
            arr[0..<ManagedStructWithBigUint::<StaticApi> as dharithri_sc::types::ManagedVecItem>::PAYLOAD_SIZE].copy_from_slice(bytes);
...
fn managed_struct_from_bytes_reader() {
    ...
    let struct_from_bytes =
        <ManagedStructWithBigUint<StaticApi> as dharithri_sc::types::ManagedVecItem>::from_byte_reader(
            |bytes| {
